...
class App:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title('충북 소통메신저 자동 사용자 선택 v1.4')
    ...
    def _tab_calib(self, frame: ttk.Frame):
        ...
        # STEP 3: 결과 없음 감지
        empty_frame = ttk.LabelFrame(frame, text='STEP 3 — 결과 없음 감지 (선택)')
        empty_frame.grid(row=2, column=0, sticky='ew', padx=10, pady=4)
        empty_frame.columnconfigure(1, weight=1)
        ...
        self.lbl_empty_pixel = tk.Label(
            empty_frame, text='미설정 (선택)', fg='#888', font=('맑은 고딕', 9)
        )
        self.lbl_empty_pixel.grid(row=1, column=0, sticky='w', padx=8)

        tk.Button(
            empty_frame, text='📍 픽셀 캡처',
            bg='#FF9800', fg='white', activebackground='#FF9800',
            relief='flat', font=('맑은 고딕', 9), padx=6, pady=3,
            cursor='hand2', command=self._capture_empty_pixel
    ...
    def _capture_empty_pixel(self):
        if not self.config.data.get('result_first_x'):
        ...
        def run():
            ...
            for i in range(3, 0, -1):
                self.root.after(
                    0, lambda i=i: self.calib_msg.config(
                        text=f'결과 없음 픽셀 캡처 {i}초 후...'
                    )
                )
                time.sleep(1)
            x = self.config.data['result_first_x']
            y = self.config.data['result_first_y']
            rgb = list(pyautogui.pixel(x, y))
            self.config.data['empty_pixel_rgb'] = rgb
            self.config.save()
            self.root.after(0, lambda: self._on_empty_pixel_captured(rgb))
            self.root.deiconify()

        threading.Thread(target=run, daemon=True).start()

    def _on_empty_pixel_captured(self, rgb):
        self.calib_msg.config(
            text=f'✅ 결과 없음 픽셀 저장: RGB({rgb[0]},{rgb[1]},{rgb[2]})'
        )
        self._refresh_calib_labels()
    ...
    def _refresh_calib_labels(self):
        ...
        if rgb:
            self.lbl_empty_pixel.config(
                text=f'✓ RGB({rgb[0]},{rgb[1]},{rgb[2]})', fg='green'
            )
        else:
            self.lbl_empty_pixel.config(text='미설정 (선택)', fg='#888')
    ...
    def _has_result(self) -> bool:
        ...
        try:
            ...
            return not all(abs(cur[i] - empty_rgb[i]) < 10 for i in range(3))
        except Exception:
            return True
